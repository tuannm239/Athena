...
def main() -> None:
    ...
    print(f"Athena CAGR  mean={statistics.mean(ath_cagr):.4f} sd={statistics.pstdev(ath_cagr):.4f} "
          f"min={min(ath_cagr):.4f} max={max(ath_cagr):.4f}")
    print(f"Athena Sharpe mean={statistics.mean(ath_sh):.4f} sd={statistics.pstdev(ath_sh):.4f} "
          f"min={min(ath_sh):.4f} max={max(ath_sh):.4f}")
    beat = statistics.mean([1.0 if p["strategies"]["Athena"]["sharpe"] > p["strategies"]["VNINDEX"]["sharpe"] else 0.0 for p in per])
    ...
    for f in range(folds):
        ...
        for p in per:
            ...
            a_all += am[f * k:(f + 1) * k]
            v_all += vmn[f * k:(f + 1) * k]
        print(f"  fold {f+1}: Athena Sharpe={sharpe_monthly(a_all):.3f}  "
    ...
    for i in range(0, len(am) - W):
        ...
        tot += 1
    print(f"  windows={tot}  Athena>VNINDEX in {wins}/{tot} ({wins/tot*100:.0f}%)  "
    ...
    boot.sort()
    print(f"  Athena Sharpe point={sharpe_monthly(am):.3f}  "
    ...
    for seed in d["seeds"][:5]:
        ...
        for mo in sorted(bm):
            ...
            if regime == "CONTRACTION":
                a_stress.append(ath); v_stress.append(mkt)
            else:
                a_calm.append(ath); v_calm.append(mkt)
    print(f"  CONTRACTION months: n={len(a_stress)}  Athena mean={_mean(a_stress):+.4f}  "
          f"VNINDEX mean={_mean(v_stress):+.4f}  edge={_mean(a_stress)-_mean(v_stress):+.4f}")
    print(f"  EXPANSION months:   n={len(a_calm)}  Athena mean={_mean(a_calm):+.4f}  "
